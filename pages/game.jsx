...
/*      
     -------------------------- VARIABLES  ---------------------------------------
*/
let off = true, inPrueba = true, oImgW = 0, oImgH = 0, charged = false, muted = false, auxPlataformas = [], imagenesBody = {}, inPause = false, initing = true, malosDie = [], chargedLevel = [], colisioned = {
    state: false,
    item: 0,
    result: 'live'
    ...
    armas = {
        ...
        die: false, last: [], new: []
    }, mxDirection = { left: false, right: false }, auxnow = 0, gameStage = 1, proyectiles = [], malosFalses = [{ posX: 150, posY: 0, widthX: 0, heightY: 0, }], dropsFalses = [], levelFalses = [{ posX: 150, posY: 0, widthX: 0, heightY: 0, }], proyectilesFalses = [], proyectilesImg = { malos: [], balas: [] }, moviendoCanvas = false, imagenesSrc = [`/img/finales/foto-de-anime-4.png`, `/img/finales/foto-de-anime-3.png`, `/img/finales/foto-de-anime-2.png`, `/img/finales/foto-de-anime-1.png`, `/img/finales/foto-de-anime-0.png`], soundToch = false, itemsImageAux = {}, soundLevels = { sfx: 0, music: 0 }, inPausetouch = false, fondos = LosFondos, obstArray = [], inLayer = 0, propsImage = PropsImage, propsAction = { chargingDireccion: 'xf', onPlataform: true, onDrop: false, speedLevel: 1, strikeLevel: 1, jumping: false, gravity: true, eating: false, jumpLevel: 1.10, gravityLevel: 1.10 }, canvas, levelGo = 1, ctx, imgArray = [], onHitSoundNow = false,
    imagenA, canvasC, ctxC, canvasB, ctxB, canvasD, ctxD, canvasE, ctxE, ctxF, canvasF, imagenes = [{ onMove: false }], imagenesEnemigos = {}, worldItems = [], timeRestart = false, levelDificulty = 20
/* ----------------------------------------------------------------------------------- */
...
const Game = (props) => {
    const { sinCargar = true, setSinCargar = console.log } = props
    /*      
         -------------------------- CONSTANTES  ---------------------------------------
    */
    ...
    const [volumenLevel, setVolumenLevel] = useState({ value: 0, mute: false })
    const [volumenEfectsLevel, setVolumenEfectsLevel] = useState({ value: 0, mute: false })
    ...
    const [stateImage, setStateImage] = useState({
        onMove: false, direccion: 'xf', posX: 0, width: 1080, height: 720, level: 1, onMobil: false
    })
    const [stateImageposXiN, setStateImageposXiN] = useState({
        posXiN: 0,
    })
    ...
    */
    const returnCharge = (retornado) => {
        ...
        charged = true
        imagenesEnemigos = retMalosImg
        worldItems = CrearItemsWorld(imagenesBody, levelGo, imagenesEnemigos, mapFloor, obst[0])
        setgetOut(true)
        setSinCargar(false)
    }
    ...
    const reboot = () => {
        ...
        propsAction = { chargingDireccion: 'xf', onPlataform: true, onDrop: false, speedLevel: 1, strikeLevel: 1, jumping: false, gravity: true, eating: false, jumpLevel: 1.10, gravityLevel: 1.10 };
        levelGo = 1;
        imgArray = [];
    ...
    const initApp = () => {
        ...
        imagenA = new Image()
        imagenA.src = '/img/body/body-body-xs-0.png'
        ...
        const hacer = async () => {
            const resMakeStage = await makeStage(false, true)
            if (resMakeStage) {
                aparecer(levelGo)
                dibujar();
    ...
    const makeStage = async (value, create, recharge) => {
        if (create) {
            ...
            for (let index = 0; index < dibujarMalos.last.length; index++) {
                ...
                if (((element.layerOnDisplay === inLayer + 1) && element.displayneed) || ((element.layerOnDisplay === inLayer) && element.displayneed)) {
                    if (element.type === 'obj') {
                        ...
                        levelFalses.push({
                            ...
                            posX: element.posX > 299 ? 290 : element.posX < 1 ? 1 : element.posX,
                            posY: element.posY,
                        ...
                        obstArray.push({
                            ...
                            imagen: obst[randomNumber],
                            killImagen: randomNumber === 0 ? obst[3] : obst[randomNumber],
                            posX: element.posX > 290 ? 290 : element.posX < 10 ? 10 : element.posX,
                            posY: element.posY,
                            widthX: element.widthX,
                            heightY: element.heightY,
                            id: element.id,
        ...
        if ((value === '+' || value === '-')) {
            ...
            /* fantasmas = [] */
            let autH = false
            let newArrayIN = dibujarMalos.last
            dibujarMalos.new.map((key, i) => {
                dibujarMalos.last.map((keyLast, iLast) => {
                    if (key.id === keyLast.id) {
                        newArrayIN[iLast] = key
                    }
                })
            })
            dibujarMalos.last = newArrayIN
            dibujarMalos.new = []
            autH = true
            levelFalses = []
            malosFalses = []
            ...
            for (let index = 0; index < dibujarMalos.last.length; index++) {
                const element = dibujarMalos.last[index];
                if (value === '+' && (element.layerOnDisplay === inLayer || element.layerOnDisplay === inLayer + 1 || element.layerOnDisplay === inLayer - 1) && element.displayneed) {
                    if (element.type === 'obj') {
                        const randomNumber = element.randomNumber
                        element.layerOnDisplay === inLayer ? levelFalses.push({
                            ...element,
                            killLayer: 0,
                            fotograma: 0,
                            randomNumber: randomNumber,
                ...
                if (value === '-' && (element.layerOnDisplay === inLayer + 1 || element.layerOnDisplay === inLayer - 1 || element.layerOnDisplay === inLayer) && element.displayneed) {
                    if (element.type === 'obj') {
                        const randomNumber = element.randomNumber
                        element.layerOnDisplay === inLayer ? levelFalses.push({
                            ...element,
        ...
        /* if (recharge) {
            setTimeout(() => {
                ...
                for (let index = 0; index < worldItems.length; index++) {
                    const element = worldItems[index];
                    if ((element.layerOnDisplay === inLayer || element.layerOnDisplay === inLayer - 1 || element.layerOnDisplay === inLayer + 1) && element.displayneed
                    ) {
                        if (element.type === 'obj') {
                            const randomNumber = element.randomNumber
    
                            obstArray.push(element)
                            ctxD.drawImage(obst[randomNumber], element.posX, element.posY, obst[randomNumber].naturalWidth / 14, obst[randomNumber].naturalHeight / 25)
                        }
                    }
                }
    ...
    const moverCanvas = async (value, die) => {
        moviendoCanvas = true
        imagenes[0].onMove = false
        const oldInlayer = inLayer
        if (value == '+' || value === '-') {
            inLayer = value === '-' && inLayer !== 0 ? inLayer - 1 : (value === '+' ? inLayer + 1 : value === '-' ? inLayer - 1 : 0)
            if (oldInlayer !== inLayer) {
                setPlayer({
                    ...player,
                    level: inLayer,
                })
                if (inLayer === 11 && gameStage === 5 && levelGo === 5) {
                    ...
                    setStateImageposXiN({
                        ...stateImageposXiN,
                        posXiN: value === '-' ? 150 : -150

                    })
                    propsImage.items[0].posX = value === '-' ? 299 : (value === '+' ? 1 : 0)
                    propsImage.posX = value === '-' ? ((inLayer + 1) * 30) - .1 : (value === '+' ? (inLayer * 30) + .1 : 0)
                    if (inLayer === 11) {
                        fantasmas = []
                        malosFalses = []
                        levelFalses = []
                        ...
                        otraImagen2.onload = (() => {
                            ...
                            laFunt(lastProp, 90)
                            const hacer = async () => {
                                const resMakeStage = await makeStage(false, false, true)
                                if (resMakeStage) {
                        ...
                        const hacer = async () => {
                            const resMakeStage = await makeStage(value, false)
                            if (resMakeStage) {
                                moviendoCanvas = false
            ...
            if (die) {
                ...
                const hacer = async () => {
                    const resMakeStage = await makeStage('-', false)
                    if (resMakeStage) {
                        moviendoCanvas = false
                        imagenes[0].onMove = true
                        return true
    ...
    */
    const efectVolumen = (start = false, value, inKey = false, inMenu = false) => {
        if (charged) {
            if (!off && charged) {
                if (!inMenu) {
                    let elValue = start ? ((inKey ? soundLevels.sfx : volumenEfectsLevel.value) / 10) : value === 'mute' ? (volumenEfectsLevel.mute ? (inKey ? soundLevels.sfx : volumenEfectsLevel.value) / 10 : 0) : (value === '+' ? ((inKey ? soundLevels.sfx : volumenEfectsLevel.value) + .5) : ((inKey ? soundLevels.sfx : volumenEfectsLevel.value) - .5)) / 10 > 0 ? (inKey ? soundLevels.sfx : volumenEfectsLevel.value - .5) / 10 : 0
                    const superValue = elValue >= 0 ? (elValue <= 1 ? elValue : 1) : 0
                    sfxObject.yaWey.volume = superValue
                    sfxObject.portraitAudio.volume = superValue
                    sfxObject.WeaponAudio[0].volume = superValue
                    sfxObject.WeaponAudio[1].volume = superValue
                    sfxObject.balaSound.volume = superValue
                    sfxObject.llantobebe.volume = superValue
                    sfxObject.joshisound.volume = superValue
                    sfxObject.joshisound2.volume = superValue
                    sfxObject.onHitSound.volume = superValue
                    sfxObject.pow.volume = superValue
                    sfxObject.sierra.volume = superValue
                    sfxObject.dolor.volume = superValue
                    sfxObject.risabebe.volume = superValue
                    sfxObject.muertebebe.volume = superValue
                    sfxObject.pass.volume = superValue
                    sfxObject.jump.volume = superValue
                    let newJoshiSound = []
                    sfxObject.joshisound3.map((key, i) => {
                        newJoshiSound[i] = key
                        newJoshiSound[i].volume = superValue
                    })
                    sfxObject.joshisound3 = newJoshiSound
                    sfxObject.itemsSound[0].volume = superValue
                    sfxObject.itemsSound[1].volume = superValue
                    const soundDatas = ['bat', 'otroBat', 'desArmado', 'lata']
                    soundDatas.map((key, i) => {
                        for (let index = 0; index < armas[key].sound.length; index++) {
                            if (armas[key].sound[index] && armas[key].sound[index].volume) {
                                armas[key].sound[index].volume = superValue
                            }
                        }
                    })
            ...
            if (value === '+') {
                const elVal = (inKey ? soundLevels.sfx : volumenEfectsLevel.value) + .5
                setVolumenEfectsLevel({
                    ...volumenEfectsLevel,
                    value: elVal <= 10 ? elVal : 10,
                    mute: false
                })
                soundLevels = {
                    ...soundLevels,
                    sfx: soundLevels.sfx + .5 <= 10 ? soundLevels.sfx + .5 : 10
                }
            }
            if (value === '-') {
                const elVal = (inKey ? soundLevels.sfx : volumenEfectsLevel.value - .5)
                ...
                setVolumenEfectsLevel({
                    ...volumenEfectsLevel,
                    value: elVal >= 0 ? elVal : 0,
                    mute: false
                })
                soundLevels = {
                    ...soundLevels,
                    sfx: soundLevels.sfx - .5 >= 0 ? (soundLevels.sfx - .5) : 0
                }
            }
        }
    ...
    const setHalfVolume = (pista, value = 'no', inKey = false, inMenu = false) => {
        if (value === 'no') {
            const revalue = ((inKey ? soundLevels.music : volumenLevel.value) / 10)
            audioPp.volume = revalue >= 0 && revalue <= 1 ? revalue : 0;
        ...
        if (value === '-' && (inKey ? soundLevels.music : volumenLevel.value) > 0) {
            const rreName = (((inKey ? soundLevels.music : volumenLevel.value) - .5))
            inMenu ? console.log : audioPp.volume = rreName / 10 >= 0 ? rreName / 10 : 0;
            setVolumenLevel({
                ...volumenLevel,
                value: rreName >= 0 ? rreName : 0, mute: false
            })
            soundLevels = {
                ...soundLevels,
                music: soundLevels.music - .5 > 0 ? soundLevels.music - .5 : 0
            }
        }
        if (value === '+' && (inKey ? soundLevels.music : volumenLevel.value) < 10) {
            const suvol = ((inKey ? soundLevels.music : volumenLevel.value) + .5)
            inMenu ? console.log : audioPp.volume = suvol / 10 <= 1 ? suvol / 10 : 1;
            setVolumenLevel({
                ...volumenLevel,
                value: suvol <= 10 ? suvol : 10, mute: false
            })
            soundLevels = {
                ...soundLevels,
                music: (soundLevels.music + .5) <= 10 ? (soundLevels.music + .5) : 10
            }
        }
 ...
 */        }
    ...
    const disparar = () => {
        try {
            ...
            sfxObject.balaSound.volume = soundLevels.sfx === 0 ? 0 : 1
    ...
    const morir = () => {
        ...
        if (actualVidas > 1 && !imagenes[0].onMove) {
            ...
            setTimeout(() => {
                ...
                ctxD.clearRect(0, 0, canvasD.width, canvasD.height)
                const res = async () => {
                    const doIt = await moverCanvas(false, true)
                    if (doIt) {
                        setPlayerGo({
                            ...playerGo,
                            go: true
    ...
    */
    const falsesMuros = async () => {
        ...
        plataformas.map((key) => {
            if ((key.layerOnDisplay === parseInt(inLayer)) || (key.layerOnDisplay === parseInt(inLayer) - 1 && key.layerOnDisplay >= 0) || (key.layerOnDisplay === parseInt(inLayer) + 1)
            ) {
    ...
    const colisonadoTrue = async () => {
        if (colisioned.result === 'die') {
            ...
            if (itsMalo) {
                ...
                ctxC.drawImage(aDibujar, 150, dibujarMalos.new[indexHere].posY, propsImage.widthX, parseInt(propsImage.heightY))
            } else {
                let isProy = false
                ...
                if (isProy) {
                    ...
                    if (isObst) {
                        ...
                        if (levelFalses[posObst].killLayer === 3) {
                            ctxE.clearRect(0, 0, canvasE.width, canvasE.height)
                            ctxE.drawImage(obst[2], 150 + ((Math.random() * 10) - 10), posYuse + ((Math.random() * 2) - 2) - 12.5, obst[2].naturalWidth / 20, obst[2].naturalHeight / 32)
                        }
                        ctxC.clearRect(0, 0, canvas.width, canvas.height)
                        ctxD.clearRect(0, 0, canvasD.width, canvasD.height)
                        ...
                        ctxC.drawImage(aDibujar, 150 - 5, posYuse - 10, 30, 25)
                        /*   if (propsImage.posX > (341 - 50)) {
                              setTimeout(() => {
                                  propsImage.posX = 0
                                  moverCanvas(false)
                              }, 4000);
        ...
        if (colisioned.result === 'live' || colisioned.result === 'rewind') {
            ...
            if (itsMalo) {
                ...
                ctxC.drawImage(aDibujar, 150, dibujarMalos.new[indexHere].posY - (dibujarMalos.new[indexHere].killLayer < 2 ? (parseInt(dibujarMalos.new[indexHere].heightY) / 2) : (parseInt(dibujarMalos.new[indexHere].heightY) / 4)), propsImage.widthX, parseInt(propsImage.heightY))
                ...
                if (isProy) {
                    ...
                    if (isObst) {
                        ...
                        ctxD.drawImage(obstacule.killImagen, posXuse, posYuse, obstacule.killImagen.naturalWidth / 14, obstacule.killImagen.naturalHeight / 25)
                        let aDibujar = propsImage.imagen[`cuted_${lastDireccion}_${obstacule.killLayer}`]
                        ctxC.drawImage(aDibujar, inPrueba ? 150 - (propsImage.widthX / 2) - 5 : posXuse - 5, posYuse - 10, 30, 25)
    ...
    */
    ...
    const distanceFuntion = (key) => {
        const realDistanceMalo = (key.layerOnDisplay * 300) + key.posX
        const realDistancePlayer = (inLayer * 300) + propsImage.items[0].posX
        const realDistancesBoth = ((realDistanceMalo - realDistancePlayer))
        if (- 150 - (key.widthX / 2) + (key.layerOnDisplay === inLayer - 1 ? -key.widthX : 0) <= realDistancesBoth && realDistancesBoth <= 150 + (key.widthX / 2)) {
            return ({
                isShow: true, distanceRange: 150 + realDistancesBoth, cuted: realDistanceMalo - realDistancePlayer > 150 ? realDistanceMalo - realDistancePlayer - 150 : 0
            })
        } else {
            return ({
                isShow: false, distanceRange: 10000, cuted: 0
            })
        }
    }
    const colisionadoFalse = async () => {
        if (propsImage.posX <= (350 - 0.5) && imagenes[0].onMove && !moviendoCanvas) {
                    ...
                    if (chokePlayer.state) {
                        for (let indd = 0; indd < chokePlayer.array.length; indd++) {
                            if (chokePlayer.array[indd].a === 'plataforma') {
                                ...
                                const darPosi = () => {
                                    let res = chokePlayer.array[indd].b.pos
                                    if (hiter === 'malo') {
                                        dibujarMalos.new.map((keySearch, iSearch) => {
                                            if (keySearch.layerOnDisplay === inLayer && (parseInt(keySearch.id) === parseInt(chokePlayer.array[indd].b.id.split('-')[0]))) {
                                                res = iSearch
                                            }
                                        })
                                    }
                                ...
                                if (hiter === ('malo') || hiter === ('proy') || hiter === ('obst') || hiter === ('bueno')) {
                                    ...
                                    if (((hiter === 'malo' || hiter === 'bueno') && chokePlayer.array[indd].c)) {
                                        if (hiter === 'malo') {
                                            ...
                                            /* onHitSound.play() */
                                            if (dibujarMalos.new[superPosi].state !== 'die' && dibujarMalos.new[superPosi].state !== 'spirit' && dibujarMalos.new[superPosi].state !== 'onDie') {
                                                dibujarMalos.new[superPosi].state = 'hit'
                                                if (!armas[armasGet.enUso].onHit) {
                                                    dibujarMalos.new[superPosi].health = dibujarMalos.new[superPosi].health - ((armas[armasGet.enUso].damage * parseInt(Math.random() * 3) + 1) * propsAction.strikeLevel)
                                                    dibujarMalos.new[superPosi].lazy = { state: true, counter: 0 }
                                                    sfxObject.joshisound2.play()
                                                    if (dibujarMalos.new[superPosi].health < 0) {
                                                        sfxObject.pow.play()
                                                        armas[armasGet.enUso].onHit = true
                                                        sfxObject.joshisound3[2].play()
                                                        fantasmas.push({
                                                            ...dibujarMalos.new[superPosi],
                                                            id: dibujarMalos.new[superPosi].id,
                                                            posX: dibujarMalos.new[superPosi].posX,
                                                            posY: dibujarMalos.new[superPosi].posY,
                                                            direccion: dibujarMalos.new[superPosi].canMove.direccion
                                                        })
                                                        dropsFalses.push(dibujarMalos.new[superPosi])
                                                        dibujarMalos.new[superPosi].state = 'die'
                                                        malosDie.push(dibujarMalos.new[superPosi].id)
                                                        setTimeout(() => {
                                                            armas[armasGet.enUso].onHit = false
                                                        }, 1000);
                                                    } else {
                                            ...
                                            if (dropsFalses.length > 0 && dropsFalses[superPosi].state === 'die' && dropsFalses[superPosi].actions.onDie.comible.state && !dropsFalses[superPosi].actions.onDie.comible.done) {
                                                if (dropsFalses.length > 0 && !propsAction.eating && !dropsFalses[superPosi].actions.onDie.comible.done) {
                                                    setplayerOnDrop({
                                                        ...playerOnDrop,
                                                        state: true
                                                    })
                                                ...
                                                if (dropsFalses.length > 0 && propsAction.eating && !dropsFalses[superPosi].actions.onDie.comible.startTaking) {
                                                    dropsFalses[superPosi].actions.onDie.comible.startTaking = true
                                                }
                                                if (dropsFalses.length > 0 && propsAction.eating && !dropsFalses[superPosi].actions.onDie.comible.done && dropsFalses[superPosi].actions.onDie.comible.startTaking && dropsFalses[superPosi].actions.onDie.comible.cantidad > 0 && !dropsFalses[superPosi].actions.onDie.comible.finishTaking) {
                                                    dropsFalses[superPosi].actions.onDie.comible.cantidad = dropsFalses[superPosi].actions.onDie.comible.cantidad - .25
                                                    let healtRes = .25 * (dropsFalses[superPosi].actions.onDie.comible.efect ? 1 : -1)
                                                    setPlayerVidas({
                                                        ...playerVidas,
                                                        health: propsImage.items[0].health.nivel + healtRes
                                                    })
                                                    propsImage.items[0].health.nivel = propsImage.items[0].health.nivel + healtRes
                                                    if (dropsFalses.length > 0 && dropsFalses[superPosi].actions.onDie.comible.cantidad <= 0) {
                                                        dropsFalses[superPosi].actions.onDie.comible.finishTaking = true
                                                        propsAction.eating = false
                                                    }

                                                }

                                                if (dropsFalses.length > 0 && dropsFalses[superPosi].actions.onDie.comible.finishTaking) {
                                                    dropsFalses[superPosi].actions.onDie.comible.done = true
                                                    propsAction.eating = false
                                                    dropsFalses = dropsFalses.filter(drop => drop !== dropsFalses[superPosi])
                                                    setplayerOnDrop({
                                                        state: false
                                                    })
                                        ...
                                        if (chokeInminente) {
                                            if (chokeInminente.state && hiter === 'malo' && propsImage.items[0].health.estado === 'inmortal') {
                                                /*   if (dibujarMalos.new[superPosi].state === 'die' && dibujarMalos.new[superPosi].actions.onDie.comible.state && !dibujarMalos.new[superPosi].actions.onDie.comible.done) {
                                                      if (!propsAction.eating && !dibujarMalos.new[superPosi].actions.onDie.comible.done) {
                                                          setplayerOnDrop({
                                                              ...playerOnDrop,
                                                              state: true
                                                          })
                                                      ...
                                                      if (propsAction.eating && !dibujarMalos.new[superPosi].actions.onDie.comible.startTaking) {
                                                          dibujarMalos.new[superPosi].actions.onDie.comible.startTaking = true
                                                      }
                                                      if (propsAction.eating && !dibujarMalos.new[superPosi].actions.onDie.comible.done && dibujarMalos.new[superPosi].actions.onDie.comible.startTaking && dibujarMalos.new[superPosi].actions.onDie.comible.cantidad > 0 && !dibujarMalos.new[superPosi].actions.onDie.comible.finishTaking) {
                                                          dibujarMalos.new[superPosi].actions.onDie.comible.cantidad = dibujarMalos.new[superPosi].actions.onDie.comible.cantidad - .05
                                                          let healtRes = .05 * (dibujarMalos.new[superPosi].actions.onDie.comible.efect ? 1 : -1)
                                                          setPlayerVidas({
                                                              ...playerVidas,
                                                              health: propsImage.items[0].health.nivel + healtRes
                                                          })
                                                          propsImage.items[0].health.nivel = propsImage.items[0].health.nivel + healtRes
                                                          if (dibujarMalos.new[superPosi].actions.onDie.comible.cantidad <= 0) {
                                                              dibujarMalos.new[superPosi].actions.onDie.comible.finishTaking = true
                                                              propsAction.eating = false
                                                          }
  
                                                      }
  
                                                      if (dibujarMalos.new[superPosi].actions.onDie.comible.finishTaking) {
                                                          dibujarMalos.new[superPosi].actions.onDie.comible.done = true
                                                          propsAction.eating = false
                                                          setplayerOnDrop({
                                                              state: false
                                                          })
                                            ...
                                            if (chokeInminente.state && propsImage.items[0].health.estado !== 'inmortal') {
                                                switch (chokeInminente.array[0].b.id.split('-')[chokeInminente.array[0].b.id.split('-').length - 1]) {
                                                    ...
                                                    case 'obst':
                                                        ...
                                                        colisioned.state = true
                                                        colisioned.item = levelFalses[superPosi].id
                                                        propsImage.items[0].health.nivel = propsImage.items[0].health.nivel - levelFalses[superPosi].damage
                                                        setPlayerVidas({
                                                            ...playerVidas,
                                                            vidas: actualVidas,
                                                            health: propsImage.items[0].health.nivel
                                                ...
                                                if (armas[armasGet.enUso].state) {
                                                    if (hiter === ('proy')) {
                                                        ...
                                                        if (proyectiles[superPosi].state !== 'onDie') {
                                                            proyectiles[superPosi].state = 'hit';
                                                            proyectiles[superPosi].hitdirection = proyectiles[superPosi].direccion === 'xf' ? (lastDireccion === 'xf' ? 'xf' : 'xb') : (lastDireccion === 'xb' ? 'xb' : 'xf')
                                                            if (!armas[armasGet.enUso].onHit) {
                                                                proyectiles[superPosi].hitDamage = (Math.random() * 4);
                                                                proyectiles[superPosi].health = armas[armasGet.enUso].onHit ? proyectiles[superPosi].health : proyectiles[superPosi].health - ((proyectiles[superPosi].health * (Math.random() * armas[armasGet.enUso].damage - 3) + 3) * propsAction.strikeLevel)
                                                                armas[armasGet.enUso].onHit = true
                                                            }
                                                            if (proyectiles[superPosi].health < 0) {
                                                                proyectiles[superPosi].state = 'die';
                                                            }
                                                        }
                                                    }
                                                    if (hiter === ('malo')) {
...
*/                    } else {
                        ...
                        if (propsImage.posX > (((inLayer + 1) * 30)) && propsImage.direccion === 'xf') {
                            imagenes[0].onMove = false
                            moviendoCanvas = true
                            moverCanvas('+', false)
                            propsAction.chargingDireccion = '+'

                        }
                        if (propsImage.items[0].posX < 150 && propsImage.direccion === 'xb' || propsImage.items[0].posX < 150 && propsImage.direccion === 'xf') {
                        }
                        if (propsImage.posX < ((inLayer * 30)) && inLayer > 0 && propsImage.direccion === 'xb') {
                            imagenes[0].onMove = false
                            moviendoCanvas = true
                            moverCanvas('-', false)
                            propsAction.chargingDireccion = '-'
                            ...
                            if (!colisioned.state) {
                                ...
                                if ((propsImage.direccion === 'xf' && propsImage.posX < 342) || ((propsImage.direccion === 'xs' || propsImage.direccion === 'xd') && propsImage.posX < 355) || (propsImage.direccion === 'xb' && propsImage.posX > 0)) {
                                    ...
                                    propsImage = {
                                        ...
                                        posX: propsImage.refreshData ? propsImage.posX : propsImage.levelPass ? propsImage.posX : !propsImage.alive ? 1 : propsImage.direccion === 'xf' && propsImage.direccion === 'xf' ? propsAction.jumping || propsImage.posY < actualFloor ? propsImage.posX + ((0.125 / (40 * (1 / (levelDificulty)))) * propsAction.speedLevel) : propsImage.posX + ((0.125 / (40 * (1 / (levelDificulty)))) * propsAction.speedLevel) : propsImage.direccion === 'xb' && propsImage.direccion === 'xb' ? propsAction.jumping || propsImage.posY < actualFloor ? propsImage.posX - ((0.125 / (40 * (1 / (levelDificulty)))) * propsAction.speedLevel) : propsImage.posX - ((0.125 / (40 * (1 / (levelDificulty)))) * propsAction.speedLevel) : propsImage.posX,
                                ...
                                setStateImageposXiN({
                                    ...stateImageposXiN,
                                    posXiN: parseInt(propsImage.items[0].posX)
                                })
                                ...
                                obstArray.map((key, i) => {
                                    const element = key
                                    const pruebi = distanceFuntion(key)
                                    const isShow = pruebi.isShow
                                    const distanceRange = pruebi.distanceRange
                                    ...
                                    if ((element.layerOnDisplay === inLayer || element.layerOnDisplay === inLayer - 1 || element.layerOnDisplay === inLayer + 1) && element.displayneed
                                        ...
                                        const randomNumber = element.randomNumber
                                        if (isShow) {
                                            ctxD.drawImage(key.imagen, distanceRange, element.posY, key.imagen.naturalWidth / 14, key.imagen.naturalHeight / 25)
                                        }
                                    }
                                })
                                malosFalses = []
                                ...
                                dibujarMalos.new.map((key, i) => {
                                    ...
                                    if (key.state !== 'die' && key.state !== 'spirit' && key.state !== 'onDie') {
                                        ...
                                        if (key.actions.shot.posibility && !key.actions.shot.state && key.layerOnDisplay === inLayer) {
                                            dibujarMalos.new[i].actions.shot.inInterval = key.actions.shot.inInterval + 1
                                        }
                                        if (key.actions.shot.posibility && key.actions.shot.state && key.layerOnDisplay === inLayer) {
                                            dibujarMalos.new[i].actions.shot.inInterval = 0
                                        }
                                        if (key.state !== 'onDie' && key.state !== 'die' && key.state !== 'spirit' && key.actions.shot.posibility && !key.actions.shot.state && key.actions.shot.inInterval === key.actions.shot.interval && key.layerOnDisplay === inLayer) {
                                        ...
                                        if (key.state === 'onDie') {
                                            ...
                                            if (isShow) {
                                                ctxE.drawImage(armas[armasGet.enUso].kills[0].imagen, distanceRange, key.posY - (key.explotionTime / 10), (armas[armasGet.enUso].kills[0].imagen.naturalWidth / 22) + (key.explotionTime / 10), (armas[armasGet.enUso].kills[0].imagen.naturalHeight / 27) + (key.explotionTime / 10))
                                            }
                                        } else {
                                            if (key.lazy.state) {
                                                ...
                                                if (isShow/*  && ((((key.canMove.direccion === 'xb' ? key.posX - (imagenready2.naturalWidth / 44) - (propsImage.items[0].posX) + distanceRange : key.posX + (imagenready2.naturalWidth / 44)) - (propsImage.items[0].posX)) + distanceRange) > 0) && ((((key.canMove.direccion === 'xb' ? key.posX - (imagenready2.naturalWidth / 44) : key.posX + (imagenready2.naturalWidth / 44)) - (propsImage.items[0].posX)) + distanceRange) < 300) */) {
                                                    ctxE.drawImage(imagenready2, (((key.canMove.direccion === 'xb' ? distanceRange + (imagenready2.naturalWidth / 44) : distanceRange - (imagenready2.naturalWidth / 44)))), key.posY - (imagenready2.naturalHeight / 27), imagenready2.naturalWidth / 22, imagenready2.naturalHeight / 27)
                                                }
                                            }
                                            if (isShow) {
                                                ctxE.drawImage(imagenready, distanceRange, key.posY, imagenready.naturalWidth / 22, imagenready.naturalHeight / 27)
                                            }
                                        }
                                        /*     if ((dibujarMalos.new[i].layerOnDisplay === inLayer && ((dibujarMalos.new[i].posX < 10 && dibujarMalos.new[i].canMove.direccion === 'xb') || (dibujarMalos.new[i].posX > 290 && dibujarMalos.new[i].canMove.direccion === 'xf'))) ||
                                                (dibujarMalos.new[i].layerOnDisplay === inLayer - 1 && ((dibujarMalos.new[i].posX < -290 && dibujarMalos.new[i].canMove.direccion === 'xb') || (dibujarMalos.new[i].posX > 10 && dibujarMalos.new[i].canMove.direccion === 'xf'))) ||
                                                (dibujarMalos.new[i].layerOnDisplay === inLayer + 1 && ((dibujarMalos.new[i].posX < 310 && dibujarMalos.new[i].canMove.direccion === 'xb') || (dibujarMalos.new[i].posX > 590 && dibujarMalos.new[i].canMove.direccion === 'xf')))) {
                                                dibujarMalos.new[i].canMove.direccion = dibujarMalos.new[i].canMove.direccion === 'xb' ? 'xf' : 'xb'
                                            } */
                                        if ((dibujarMalos.new[i].posX < 0 && dibujarMalos.new[i].canMove.direccion === 'xb') || (dibujarMalos.new[i].posX > 299 && dibujarMalos.new[i].canMove.direccion === 'xf')) {
                                            dibujarMalos.new[i].canMove.direccion = dibujarMalos.new[i].canMove.direccion === 'xb' ? 'xf' : 'xb'
                                        }
                                        ...
                                        if (isShow) {
                                            ctxE.drawImage(position.die.imagen, distanceRange, key.posY, position.die.imagen.naturalWidth / 22, position.die.imagen.naturalHeight / 27)
                                        }
                                    }
                                    if (key.layerOnDisplay === inLayer && key.state !== 'die' && key.state !== 'onDie' && key.state !== 'spirit') {
                                        malosFalses.push(dibujarMalos.new[i])
                                    }
                                })
                                /*    dibujarMalos.new.map((keySearch, iSearch) => {
                                       ...
                                       let dieIs = false
                                       malosDie.map((keyMai, iMap) => {
                                           if ((parseInt(keyMai) === parseInt(keySearch.id))) {
                                               dieIs = true
                                           }
                                       })
                                       if (!dieIs && keySearch.layerOnDisplay === inLayer) {
                                           malosFalsesAux.push({
                                               ...keySearch,
                                               id: keySearch.id,
                                               posX: keySearch.posX,
                                               posY: keySearch.posY,
                                               widthX: keySearch.widthX,
                                               heightY: parseInt(keySearch.heightY),
                                           })
                                       }
                                   }) */
                                ...
                                proyectiles.map((key, i) => {
                                    ...
                                    const isShow = pruebi.isShow
                                    const distanceRange = pruebi.distanceRange
                                    isShow && ctxE.drawImage(key.imagen[key.direccion === 'xf' ? 1 : 0], distanceRange, key.posY, key.widthX, parseInt(key.heightY))
                                    ...
                                    if ((key.state !== 'die' && key.state !== 'onDie' && key.state !== 'spirit') && (key.posX < 300 && (key.posX + key.widthX) > 0) && (key.posY > 0 && key.posY > 0)) {
                                        ...
                                        if ((key.layerOnDisplay === inLayer ||key.layerOnDisplay === inLayer - 1 || key.layerOnDisplay === inLayer + 1) && (key.state !== 'die' && key.state !== 'onDie' && key.state !== 'spirit') && (key.posX > 300 || (key.posX + key.widthX) < 0) && (key.posY > 0 && key.posY > 0)) {
                                            proyectilesFalses.push({
                                                ...key,
                                                damageFor: key.damageFor,
                                                id: key.id,
                                                layerOnDisplay: key.layerOnDisplay + (key.posX > 300 ? 1 : -1),
                                                state: key.posX,
                                                posX: key.posX > 300 ? 1 : 299,
                                                posY: key.posY,
                                                widthX: key.widthX,
                                                heightY: parseInt(key.heightY),
                                                health: key.health,
                                ...
                                fantasmas.map((keySpirit, iGhost) => {
                                    ...
                                    const pruebi = distanceFuntion(keySpirit)
                                    const isShow = pruebi.isShow
                                    const distanceRange = pruebi.distanceRange /* (realDistanceMalo - realDistancePlayer <= 150 && realDistanceMalo - realDistancePlayer >= 0) || (realDistancePlayer - realDistanceMalo <= 150 && realDistancePlayer - realDistanceMalo >= 0) ? (keySpirit.posX >= 150 ? propsImage.items[0].posX >= 150 ? -150 : 150 : propsImage.items[0].posX >= 150 ? 150 : 150) : (realDistanceMalo >= realDistancePlayer ? 300 : -300) */

                                    if (keySpirit.posY > 0) {
                                        imagenesEnemigos.map((key2, i) => {
                                            if (key2.direccion === `spirit_${keySpirit.direccion}_0`) {
                                                positionSpirit.spirit.imagen = key2.imagen
                                            }
                                        })

                                        fantasmas[iGhost].posY = fantasmas[iGhost].posY - (1.25 / 3)
                                        if (isShow) {
                                            ctxE.drawImage(positionSpirit.spirit.imagen, distanceRange, keySpirit.posY, positionSpirit.spirit.imagen.naturalWidth / 22, positionSpirit.spirit.imagen.naturalHeight / 27)
                                            newFantasmas.push(fantasmas[iGhost])
                                        }

                                    }
                                })
                                fantasmas = newFantasmas
                                ...
                                if (itemsGet.enUso !== 'ninguno') {
                                    ...
                                    ctxE.drawImage(playerClothes, inPrueba ? 150 - (propsImage.widthX) : propsImage.levelPass ? psx : propsImage.refreshData ? 10 : !propsImage.alive ? 0 : psx - 8, propsImage.direccion === 'xd' ? propsImage.posY + (parseInt(propsImage.heightY) / 2) : propsImage.posY - 7, propsImage.widthX + 16, propsImage.direccion === 'xd' ? (parseInt(propsImage.heightY) / 2) : parseInt(propsImage.heightY) + 14)
                                }
                                if (armas[armasGet.enUso].state) {
                                    ...
                                    if (armas[armasGet.enUso].layer < 13) {
                                        ...
                                        ctxE.drawImage(armas[armasGet.enUso].imagenes[posLayer].imagen, (inPrueba ? 150 - (propsImage.widthX / 2) : psx) - 15, propsImage.posY - (parseInt(propsImage.heightY) / 2), armas[armasGet.enUso].imagenes[posLayer].widthX, armas[armasGet.enUso].imagenes[posLayer].heightY);
                                    } else {
                                        let posLayer = 0
                                        let cualLayer = 0
                                        armas[armasGet.enUso].imagenes.map((key2, i) => {
                                ...
                                if (propsImage.items[0].health.estado === 'inmortal') {
                                    ...
                                    ctxC.drawImage(playerClothes, inPrueba ? 150 - (propsImage.widthX / 2) - 8 : propsImage.levelPass ? psx : propsImage.refreshData ? 10 : !propsImage.alive ? 0 : psx - 8, propsImage.direccion === 'xd' ? propsImage.posY + (parseInt(propsImage.heightY) / 2) : propsImage.posY - 7, propsImage.widthX + 16, propsImage.direccion === 'xd' ? (parseInt(propsImage.heightY) / 2) : parseInt(propsImage.heightY) + 14)
                                }
                                ...
                                plataformas.map((key) => {
                                    ...
                                    const isShow = pruebi.isShow
                                    const distanceRange = pruebi.distanceRange
                                    const cuted = pruebi.cuted
                                    if (isShow
                                    ) {
                                        ctxF.beginPath()
                                        ctxF.fillStyle = 'brown'
                                        ctxF.fillRect(distanceRange + cuted, key.posY, key.widthX - cuted, key.heightY)
                                        ctxF.closePath()

                                    }

                                })
                                ctxC.drawImage(aDibujar, inPrueba ? 150 - (propsImage.widthX / 2) : propsImage.levelPass ? psx : propsImage.refreshData ? 10 : !propsImage.alive ? 0 : psx, propsImage.direccion === 'xd' ? propsImage.posY + (parseInt(propsImage.heightY) / 2) : propsImage.posY, propsImage.widthX, propsImage.direccion === 'xd' ? (parseInt(propsImage.heightY) / 2) : parseInt(propsImage.heightY))
                                if (propsImage.items[0].health.estado === 'inmortal' && !propsAction.eating) {
                                    ...
                                    ctxC.drawImage(playerClothes, (inPrueba ? 150 - (propsImage.widthX / 2) + (armas[armasGet.enUso].state ? -2 : propsImage.direccion === 'xb' ? -4 + (1 * (propsImage.widthX / 2)) : propsImage.direccion === 'xf' ? -17 : -12) : psx),
                                ...
                                dibujarMalos.new.map((key, i) => {
                                    if (colisioned.item === key.id) {
                                        key.imagen.map((key2, ia) => {
                                            ...
                                            if (isShow) {
                                                ctxE.drawImage(key.imagen[position.malo].imagen, distanceRange, key.posY, key.imagen[position.malo].imagen.naturalWidth / 22, key.imagen[position.malo].imagen.naturalHeight / 27)
                                            }
                                        })
                                    }
                                })
                                let aDibujar = propsImage.imagen[`die_${propsImage.direccion}_${'0'}`]
                                ctxC.drawImage(aDibujar, inPrueba ? 150 - (propsImage.widthX / 2) : psx, propsImage.posY, propsImage.widthX, parseInt(propsImage.heightY))
                            }
                        }
                    }
                    return { status: true }
                ...
                <><ChargeComponent returnCharge={returnCharge} getOut={getOut} /></> : <>
                    <div className={onMobil ? `IDiv-main column bgGame bgGame-mobil relativeCanvasContainer ${!gameStart ? '' : 'horizontal-mode'}` : `IDiv-main column bgGame relativeCanvasContainer ${!gameStart ? '' : 'horizontal-mode'}`}>
                        ...
                        <div className={gameStart ? '' : 'ocult'}>
                            ...
                            <canvas className={`${onMobil ? 'bgUrlmobil' : 'bgUrl'}-${(playerStage.stage + 1)} lienzoBg lienzo-${stateImage.posX}-${stateImageposXiN.posXiN} lienzoW-${parseInt(stateImage.width)} ${onMobil ? !fullScreen ? `lienzoHM` : `lienzoH-${parseInt(stateImage.height)}` : `lienzoH-${parseInt(stateImage.height)}`}`} id="canvas-Pp">
                            </canvas>
